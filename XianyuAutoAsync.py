...
class XianyuLive:
    ...
    def __init__(self, cookies_str=None, cookie_id: str = "default", user_id: int = None):
        ...
        # 订单处理并发控制
        self.order_semaphore = asyncio.Semaphore(5)  # 最多5个并发订单处理任务
        ...
        try:
            ...
            async with order_lock:
                ...
                async with self.order_semaphore:
                    logger.info(f'[{msg_time}] 【{self.cookie_id}】获取订单处理信号量，当前并发数: {5 - self.order_semaphore._value}')
    ...
    async def fetch_order_detail_info(self, order_id: str, item_id: str = None, buyer_id: str = None, debug_headless: bool = None, force_refresh: bool = False):
        ...
        async with order_detail_lock:
            ...
            try:
                ...
                # 从浏览器实例池获取实例
                fetcher = await browser_pool.get_instance(self.cookie_id, cookie_string, headless=headless_mode)
                # 异步获取订单详情（使用当前账号的cookie）
                result = await fetcher.fetch_order_detail(order_id, force_refresh=force_refresh)
                ...
                if result:
                    ...
                    # 获取收货人信息
                    consignee_name = result.get('consignee_name', '')
                    consignee_phone = result.get('consignee_phone', '')
                    consignee_address = result.get('consignee_address', '')
            ...
            while True:
                try:
                    ...
                    if consecutive_failures >= max_failures:
                        ...
                        # 心跳连续失败，标记Cookie可能已失效
                        try:
                            from db_manager import db_manager
                            db_manager.update_cookie_validation_status(
                                self.cookie_id, 
                                False, 
                                f"心跳连续失败{max_failures}次，Cookie可能已失效"
                            )
                            logger.warning(f"【{self.cookie_id}】心跳连续失败，标记Cookie为失效")
                        except Exception as e:
                            logger.debug(f"更新心跳失败状态失败: {self._safe_str(e)}")
    ...
    async def handle_heartbeat_response(self, message_data):
        ...
        try:
            if message_data.get("code") == 200:
                ...
                # 心跳成功，更新Cookie验证状态为有效
                try:
                    from db_manager import db_manager
                    # 检查当前状态，避免频繁更新数据库
                    current_status = db_manager.get_cookie_validation_status(self.cookie_id)
                    if not current_status or current_status.get('is_valid') != True:
                        db_manager.update_cookie_validation_status(
                            self.cookie_id, 
                            True, 
                            "心跳包正常，Cookie有效"
                        )
                        logger.info(f"【{self.cookie_id}】心跳验证成功，更新Cookie状态为有效")
                except Exception as e:
                    logger.debug(f"更新心跳验证状态失败: {self._safe_str(e)}")
