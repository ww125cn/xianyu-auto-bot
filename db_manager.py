...
class DBManager:
    ...
    def __init__(self, db_path: str = None):
        ...
        # 密码加密上下文 - 使用 SHA256 代替 bcrypt 以避免库兼容性问题
    ...
    def init_db(self):
        ...
        try:
            ...
            # 检查并添加收货人相关列
            try:
                self._execute_sql(cursor, "SELECT consignee_name FROM orders LIMIT 1")
            except sqlite3.OperationalError:
                # consignee_name 列不存在，需要添加
                logger.info("正在为 orders 表添加 consignee_name 列...")
                self._execute_sql(cursor, "ALTER TABLE orders ADD COLUMN consignee_name TEXT")
                logger.info("orders 表 consignee_name 列添加完成")

            try:
                self._execute_sql(cursor, "SELECT consignee_phone FROM orders LIMIT 1")
            except sqlite3.OperationalError:
                # consignee_phone 列不存在，需要添加
                logger.info("正在为 orders 表添加 consignee_phone 列...")
                self._execute_sql(cursor, "ALTER TABLE orders ADD COLUMN consignee_phone TEXT")
                logger.info("orders 表 consignee_phone 列添加完成")

            try:
                self._execute_sql(cursor, "SELECT consignee_address FROM orders LIMIT 1")
            except sqlite3.OperationalError:
                # consignee_address 列不存在，需要添加
                logger.info("正在为 orders 表添加 consignee_address 列...")
                self._execute_sql(cursor, "ALTER TABLE orders ADD COLUMN consignee_address TEXT")
                logger.info("orders 表 consignee_address 列添加完成")
    ...
    def check_and_upgrade_db(self, cursor):
        ...
        try:
            ...
            # 升级到版本1.6 - 为users表添加is_admin字段
            if current_version < "1.6":
                logger.info("开始升级数据库到版本1.6...")
                self.upgrade_users_table_for_is_admin_field(cursor)
                self.set_system_setting("db_version", "1.6", "数据库版本号")
                logger.info("数据库升级到版本1.6完成")

            # 升级到版本1.7 - 为cookie_status表添加验证状态字段
            if current_version < "1.7":
                logger.info("开始升级数据库到版本1.7...")
                self.upgrade_cookie_status_table_for_validation(cursor)
    ...
    def update_admin_user_id(self, cursor):
        ...
        try:
            ...
            # 首先检查并添加is_admin字段（如果不存在）
            try:
                cursor.execute("SELECT is_admin FROM users LIMIT 1")
            except sqlite3.OperationalError:
                logger.info("users表is_admin字段不存在，先添加...")
                cursor.execute("ALTER TABLE users ADD COLUMN is_admin BOOLEAN DEFAULT FALSE")
                logger.info("users表is_admin字段添加成功")
            ...
            if not admin_exists:
                # 首次创建admin用户，设置默认密码
                # 截断密码到72字节，以符合bcrypt的要求
                password = "admin123"
                password = password[:72]  # bcrypt限制密码长度不超过72字节
                default_password_hash = self.pwd_context.hash(password)
                cursor.execute('''
                ...
                # 确保现有admin用户的is_admin字段为True
                cursor.execute('''
                UPDATE users SET is_admin = 1 WHERE username = 'admin'
                ''')
                logger.info("确保admin用户的is_admin字段为True")
    ...
    def upgrade_users_table_for_is_admin_field(self, cursor):
        """升级users表添加is_admin字段"""
        try:
            logger.info("开始为users表添加is_admin字段...")
            ...
            try:
                ...
                # 确保admin用户的is_admin字段为True
                self._execute_sql(cursor, "UPDATE users SET is_admin = 1 WHERE username = 'admin'")
                logger.info("设置admin用户的is_admin字段为True")

            logger.info("✅ users表is_admin字段升级完成")
            return True
        except Exception as e:
            logger.error(f"升级users表is_admin字段失败: {e}")
            raise

    def upgrade_cookie_status_table_for_validation(self, cursor):
        """升级cookie_status表添加验证状态字段"""
        try:
            logger.info("开始为cookie_status表添加验证状态字段...")

            # 为cookie_status表添加is_valid字段（如果不存在）
            try:
                self._execute_sql(cursor, "SELECT is_valid FROM cookie_status LIMIT 1")
                logger.info("cookie_status表is_valid字段已存在")
            except sqlite3.OperationalError:
                self._execute_sql(cursor, "ALTER TABLE cookie_status ADD COLUMN is_valid BOOLEAN DEFAULT NULL")
                logger.info("为cookie_status表添加is_valid字段")

            # 为cookie_status表添加validation_message字段（如果不存在）
            try:
                self._execute_sql(cursor, "SELECT validation_message FROM cookie_status LIMIT 1")
                logger.info("cookie_status表validation_message字段已存在")
            except sqlite3.OperationalError:
                self._execute_sql(cursor, "ALTER TABLE cookie_status ADD COLUMN validation_message TEXT")
                logger.info("为cookie_status表添加validation_message字段")

            # 为cookie_status表添加last_validated_at字段（如果不存在）
            try:
                self._execute_sql(cursor, "SELECT last_validated_at FROM cookie_status LIMIT 1")
                logger.info("cookie_status表last_validated_at字段已存在")
            except sqlite3.OperationalError:
                self._execute_sql(cursor, "ALTER TABLE cookie_status ADD COLUMN last_validated_at TIMESTAMP")
                logger.info("为cookie_status表添加last_validated_at字段")

            logger.info("✅ cookie_status表验证状态字段升级完成")
            return True
        except Exception as e:
            logger.error(f"升级cookie_status表验证状态字段失败: {e}")
            raise
    ...
    def _begin_transaction(self, cursor):
        """开始事务"""
    ...
    def save_cookie(self, cookie_id: str, cookie_value: str, user_id: int = None) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("保存Cookie失败: cookie_id必须是非空字符串")
                    return False
                if not cookie_value or not isinstance(cookie_value, str):
                    logger.error("保存Cookie失败: cookie_value必须是非空字符串")
                    return False
                if user_id is not None and not isinstance(user_id, int):
                    logger.error("保存Cookie失败: user_id必须是整数")
                    return False
    ...
    def delete_cookie(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("删除Cookie失败: cookie_id必须是非空字符串")
                    return False
    ...
    def get_cookie(self, cookie_id: str) -> Optional[str]:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("获取Cookie失败: cookie_id必须是非空字符串")
                    return None
    ...
    def get_all_cookies(self, user_id: int = None) -> Dict[str, str]:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if user_id is not None and not isinstance(user_id, int):
                    logger.error("获取所有Cookie失败: user_id必须是整数")
                    return {}
    ...
    def get_cookie_by_id(self, cookie_id: str) -> Optional[Dict[str, str]]:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("获取Cookie信息失败: cookie_id必须是非空字符串")
                    return None
    ...
    def get_cookie_details(self, cookie_id: str) -> Optional[Dict[str, any]]:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("获取Cookie详细信息失败: cookie_id必须是非空字符串")
                    return None
                ...
                if result:
                    ...
                    if password:
                        try:
                            password = self.fernet.decrypt(password.encode()).decode()
                        except Exception as e:
    ...
    def update_cookie_validation_status(self, cookie_id: str, is_valid: bool, message: str = "") -> bool:
        """更新Cookie的验证状态
        ...
        with self.lock:
            try:
                ...
                # 检查cookie_status记录是否存在
                self._execute_sql(cursor, "SELECT cookie_id FROM cookie_status WHERE cookie_id = ?", (cookie_id,))
                ...
                if exists:
                    ...
                    self._execute_sql(
                        cursor,
                        """UPDATE cookie_status 
                           SET is_valid = ?, validation_message = ?, last_validated_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP 
                           WHERE cookie_id = ?""",
                        (int(is_valid), message, cookie_id)
                    )
                else:
                    # 创建新记录
                    self._execute_sql(
                        cursor,
                        """INSERT INTO cookie_status (cookie_id, enabled, is_valid, validation_message, last_validated_at, updated_at) 
                           VALUES (?, 1, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)""",
                        (cookie_id, int(is_valid), message)
                    )
                
                self.conn.commit()
                logger.info(f"更新账号 {cookie_id} 验证状态: {'有效' if is_valid else '无效'}, 消息: {message}")
                return True
            except Exception as e:
                logger.error(f"更新账号验证状态失败: {e}")
                return False

    def get_cookie_validation_status(self, cookie_id: str) -> Optional[Dict[str, any]]:
        """获取Cookie的验证状态
        ...
        Returns:
            验证状态字典，包含 is_valid, validation_message, last_validated_at 等字段
        """
        with self.lock:
            try:
                cursor = self.conn.cursor()
                self._execute_sql(
                    cursor,
                    """SELECT is_valid, validation_message, last_validated_at, enabled 
                       FROM cookie_status WHERE cookie_id = ?""",
                    (cookie_id,)
                )
                result = cursor.fetchone()
                if result:
                    return {
                        'is_valid': bool(result[0]) if result[0] is not None else None,
                        'validation_message': result[1] or '',
                        'last_validated_at': result[2],
                        'enabled': bool(result[3]) if result[3] is not None else True
                    }
                return None
            except Exception as e:
                logger.error(f"获取账号验证状态失败: {e}")
                return None

    def get_all_cookies_with_validation_status(self, user_id: int = None) -> Dict[str, Dict]:
        """获取所有Cookie及其验证状态
        
        Args:
            user_id: 用户ID，如果为None则返回所有Cookie
            
        Returns:
            字典，key为cookie_id，value包含cookie信息和验证状态
        """
        with self.lock:
            try:
                cursor = self.conn.cursor()
                ...
                if user_id is not None:
                    self._execute_sql(
                        cursor,
                        """SELECT c.id, c.value, cs.is_valid, cs.validation_message, cs.last_validated_at, cs.enabled
                           FROM cookies c
                ...
                for row in cursor.fetchall():
                    result[row[0]] = {
                        'id': row[0],
                        'value': row[1],
                        'is_valid': bool(row[2]) if row[2] is not None else None,
                        'validation_message': row[3] or '',
                        'last_validated_at': row[4],
                        'enabled': bool(row[5]) if row[5] is not None else True
                    }
                return result
            except Exception as e:
                logger.error(f"获取所有Cookie验证状态失败: {e}")
                return {}
    ...
    def save_keywords_with_item_id(self, cookie_id: str, keywords: List[Tuple[str, str, str]]) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("保存关键字失败: cookie_id必须是非空字符串")
                    return False
                if not keywords or not isinstance(keywords, list):
                    logger.error("保存关键字失败: keywords必须是非空列表")
                    return False
    ...
    def save_text_keywords_only(self, cookie_id: str, keywords: List[Tuple[str, str, str]]) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("保存文本关键字失败: cookie_id必须是非空字符串")
                    return False
                if not keywords or not isinstance(keywords, list):
                    logger.error("保存文本关键字失败: keywords必须是非空列表")
                    return False
    ...
    def save_image_keyword(self, cookie_id: str, keyword: str, image_url: str, item_id: str = None) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("保存图片关键词失败: cookie_id必须是非空字符串")
                    return False
                if not keyword or not isinstance(keyword, str):
                    logger.error("保存图片关键词失败: keyword必须是非空字符串")
                    return False
                if not image_url or not isinstance(image_url, str):
                    logger.error("保存图片关键词失败: image_url必须是非空字符串")
                    return False
    ...
    def update_keyword_image_url(self, cookie_id: str, keyword: str, new_image_url: str) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("更新关键词图片URL失败: cookie_id必须是非空字符串")
                    return False
                if not keyword or not isinstance(keyword, str):
                    logger.error("更新关键词图片URL失败: keyword必须是非空字符串")
                    return False
                if not new_image_url or not isinstance(new_image_url, str):
                    logger.error("更新关键词图片URL失败: new_image_url必须是非空字符串")
                    return False
    ...
    def delete_keyword_by_index(self, cookie_id: str, index: int) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("删除关键词失败: cookie_id必须是非空字符串")
                    return False
                if not isinstance(index, int) or index < 0:
                    logger.error("删除关键词失败: index必须是非负整数")
                    return False
    ...
    def save_default_reply(self, cookie_id: str, enabled: bool, reply_content: str = None, reply_once: bool = False, reply_image_url: str = None, item_id: str = None):
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("保存默认回复设置失败: cookie_id必须是非空字符串")
                    return False
                if not isinstance(enabled, bool):
                    logger.error("保存默认回复设置失败: enabled必须是布尔值")
                    return False
                if reply_content is not None and not isinstance(reply_content, str):
                    logger.error("保存默认回复设置失败: reply_content必须是字符串")
                    return False
                if not isinstance(reply_once, bool):
                    logger.error("保存默认回复设置失败: reply_once必须是布尔值")
                    return False
                if reply_image_url is not None and not isinstance(reply_image_url, str):
                    logger.error("保存默认回复设置失败: reply_image_url必须是字符串")
                    return False
                if item_id is not None and not isinstance(item_id, str):
                    logger.error("保存默认回复设置失败: item_id必须是字符串")
                    return False
                ...
                # 先检查是否存在
                if item_id:
                    self._execute_sql(cursor, 'SELECT id FROM default_replies WHERE cookie_id = ? AND item_id = ?', (cookie_id, item_id))
                else:
                    self._execute_sql(cursor, 'SELECT id FROM default_replies WHERE cookie_id = ? AND item_id IS NULL', (cookie_id,))
    ...
    def set_message_notification(self, cookie_id: str, channel_id: int, enabled: bool = True) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("设置消息通知失败: cookie_id必须是非空字符串")
                    return False
                if not isinstance(channel_id, int) or channel_id < 0:
                    logger.error("设置消息通知失败: channel_id必须是非负整数")
                    return False
                if not isinstance(enabled, bool):
                    logger.error("设置消息通知失败: enabled必须是布尔值")
                    return False
        ...
        with self.lock:
            try:
                ...
                if not isinstance(notification_id, int) or notification_id < 0:
                    logger.error("删除消息通知配置失败: notification_id必须是非负整数")
                    return False
    ...
    def delete_account_notifications(self, cookie_id: str) -> bool:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if not cookie_id or not isinstance(cookie_id, str):
                    logger.error("删除账号通知配置失败: cookie_id必须是非空字符串")
                    return False
    ...
    def export_backup(self, user_id: int = None) -> Dict[str, any]:
        ...
        with self.lock:
            try:
                # 验证输入数据
                if user_id is not None and not isinstance(user_id, int):
                    logger.error("导出备份失败: user_id必须是整数")
            ...
            try:
                ...
                for table_name, table_data in data.items():
                    ...
                    if not re.match(r'^[a-zA-Z0-9_]+$', table_name):
                        logger.warning(f"非法表名: {table_name}，跳过导入")
                        continue
                    ...
                    for col in columns:
                        if not re.match(r'^[a-zA-Z0-9_]+$', col):
                            logger.warning(f"非法列名: {col}，跳过导入")
                            validated_columns = []
                            break
                        validated_columns.append(f"[{col}]")  # 使用方括号包裹列名，防止SQL注入
                    ...
                    if table_name == 'system_settings':
                        ...
                        cursor.executemany(f"INSERT INTO [{table_name}] ({columns_str}) VALUES ({placeholders})", rows)
    ...
    def save_session(self, token: str, user_id: int, username: str, is_admin: bool, expires_at: float) -> bool:
        """保存会话到数据库"""
        with self.lock:
            try:
                cursor = self.conn.cursor()
                ...
                cursor.execute('''
                INSERT OR REPLACE INTO sessions (token, user_id, username, is_admin, expires_at)
                VALUES (?, ?, ?, ?, ?)
                ''', (token, user_id, username, is_admin, expires_at_str))
                self.conn.commit()
                return True
            except Exception as e:
    ...
    def get_session(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        with self.lock:
            try:
                ...
                if result:
                    # 检查会话是否过期
                    expires_at_str = result[3]
                    expires_at = time.mktime(time.strptime(expires_at_str, '%Y-%m-%d %H:%M:%S'))
    ...
    def update_user_password(self, username: str, new_password: str) -> bool:
        ...
        with self.lock:
            try:
                cursor = self.conn.cursor()
                # 截断密码到72字节，以符合bcrypt的要求
                new_password = new_password[:72]  # bcrypt限制密码长度不超过72字节
...
© 2025 闲鱼自动回复系统"""
    ...
    def insert_or_update_order(self, order_id: str, item_id: str = None, buyer_id: str = None,
                              ...
                              is_bargain: bool = None, consignee_name: str = None, consignee_phone: str = None,
            ...
            try:
                ...
                if existing:
                    ...
                    if consignee_name is not None:
                        update_fields.append("consignee_name = ?")
                        update_values.append(consignee_name)
                    if consignee_phone is not None:
                        update_fields.append("consignee_phone = ?")
                        update_values.append(consignee_phone)
                    if consignee_address is not None:
                        update_fields.append("consignee_address = ?")
                        update_values.append(consignee_address)
                    ...
                    ''', (order_id, item_id, buyer_id, spec_name, spec_value,
                          ...
                          1 if is_bargain else 0, consignee_name, consignee_phone, consignee_address))
        ...
        with self.lock:
            try:
                ...
                SELECT order_id, item_id, buyer_id, spec_name, spec_value,
                       quantity, amount, order_status, cookie_id, is_bargain, consignee_name, consignee_phone, consignee_address, created_at, updated_at
                ...
                if row:
                    return {
                        ...
                        'consignee_name': row[10],
                        'consignee_phone': row[11],
    ...
    def get_orders_by_cookie(self, cookie_id: str, limit: int = 100):
        ...
        with self.lock:
            try:
                ...
                cursor.execute('''
                SELECT order_id, item_id, buyer_id, spec_name, spec_value,
                       quantity, amount, order_status, is_bargain, consignee_name, consignee_phone, consignee_address, created_at, updated_at
                ...
                for row in cursor.fetchall():
                    orders.append({
                        ...
                        'consignee_phone': row[10],
                        'consignee_address': row[11],
                        'created_at': row[12],
                        'updated_at': row[13]
                    })
    ...
    def get_orders_by_user(self, user_id: int, limit: int = 1000):
        """根据用户ID获取所有订单列表"""
        with self.lock:
            try:
                cursor = self.conn.cursor()
                ...
                SELECT o.order_id, o.item_id, o.buyer_id, o.spec_name, o.spec_value,
                       o.quantity, o.amount, o.order_status, o.cookie_id, o.is_bargain, o.consignee_name, o.consignee_phone, o.consignee_address, o.created_at, o.updated_at
                FROM orders o
