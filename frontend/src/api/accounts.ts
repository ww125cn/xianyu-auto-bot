...
// 验证单个Cookie的有效性（增加超时时间到3分钟）
export const validateAccount = (id: string): Promise<{ success: boolean; is_valid?: boolean; message?: string }> => {
  return post(`/cookies/${id}/validate`, {}, { timeout: 180000 })
}

// 验证所有Cookie（增加超时时间到10分钟）
export const validateAllAccounts = (): Promise<{ success: boolean; message?: string; results?: Record<string, { is_valid: boolean; message: string }> }> => {
  return post('/cookies/validate-all', {}, { timeout: 600000 })
}
