...
export function Accounts() {
  ...
  // 验证所有账号Cookie
  const handleValidateAllAccounts = async () => {
    setValidatingAll(true)
    try {
      const result = await validateAllAccounts()
      if (result.success) {
        const validCount = Object.values(result.results || {}).filter(r => r.is_valid).length
        const invalidCount = Object.values(result.results || {}).filter(r => !r.is_valid).length
        addToast({
          type: 'success',
  ...
  // 获取Cookie状态显示
  const getCookieStatusDisplay = (account: AccountWithKeywordCount) => {
    if (account.is_valid === null || account.is_valid === undefined) {
      return {
        icon: <ShieldQuestion className="w-3.5 h-3.5 text-slate-400" />,
        text: '未验证',
    ...
    <div className="space-y-4">
      ...
      <div className="page-header flex-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={handleValidateAllAccounts}
            disabled={validatingAll}
            className="btn-ios-secondary"
            title="验证所有账号Cookie状态"
          >
            {validatingAll ? (
              ...
              {accounts.length === 0 ? (
                ...
                accounts.map((account) => (
                  <tr key={account.id}>
                    ...
                    <td>
                      {(() => {
                        const status = getCookieStatusDisplay(account)
                        return (
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs font-medium ${status.bgClass} ${status.className}`}>
                              {status.icon}
                              {status.text}
                            </span>
                            <button
                            ...
                            >
                              {validatingAccount === account.id ? (
                                ...
                                <RefreshCcw className="w-3.5 h-3.5 text-slate-400 hover:text-blue-500" />
                              )}
                            </button>
                          </div>
                        )
