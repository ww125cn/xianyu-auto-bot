...
# 存储请求记录
request_records = defaultdict(list)  # {ip: [(timestamp, endpoint), ...]}
...
async def cleanup_expired_records():
    async with cleanup_lock:
        ...
        for ip, records in list(request_records.items()):
            # 保留最近10分钟的记录
            request_records[ip] = [r for r in records if current_time - r[0] < 600]
            # 如果没有记录了，删除该IP
...
def verify_token(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> Optional[Dict[str, Any]]:
    ...
    # 使用数据库验证会话
    session_info = db_manager.get_session(token)
    if not session_info:
        return None
...
async def rate_limit_middleware(request, call_next):
    # 获取客户端IP - 只使用直连IP，不考虑X-Forwarded-For头部
    # 这样可以防止通过伪造X-Forwarded-For头部绕过速率限制
    ...
    # 清理过期记录
    if random.random() < 0.1:  # 10%的概率清理过期记录
    ...
    if client_ip in request_records:
        ...
        # 更新记录
        request_records[client_ip] = recent_requests
...
# 从环境变量读取API秘钥，默认值为空
API_SECRET_KEY = os.environ.get('API_SECRET_KEY', '')  # 保留作为后备
...
def verify_api_key(api_key: str) -> bool:
    ...
    try:
        # 优先从环境变量获取
        qq_secret_key = os.environ.get('QQ_REPLY_SECRET_KEY')
        
        # 如果环境变量中没有配置，从系统设置中获取
...
def get_public_system_settings():
    ...
    try:
        ...
        # 只返回公开的配置项
        public_keys = {"registration_enabled", "login_captcha_enabled"}
...
def export_backup(
    ...
    try:
        ...
        # 验证 token
        from db_manager import db_manager
        session_info = db_manager.get_session(token_to_verify)
        if not session_info:
...
def download_database_backup(
    ...
    try:
        ...
        # 检查是否是管理员
        if session_info['username'] != ADMIN_USERNAME:
            raise HTTPException(status_code=403, detail="需要管理员权限")
        
        admin_user = session_info
...
def get_user_orders(current_user: Dict[str, Any] = Depends(get_current_user)):
    ...
    try:
        ...
        # 一次访问获取所有订单数据
        all_orders = db_manager.get_orders_by_user(user_id, limit=1000)
...
async def validate_cookie_api(cookie_id: str, current_user: Dict[str, Any] = Depends(get_current_user)):
    """手动验证指定Cookie的有效性"""
    try:
        # 检查cookie是否属于当前用户
        user_id = current_user['user_id']
        ...
        # 执行验证
        from utils.cookie_validator import validate_cookie_now
        is_valid, message = await validate_cookie_now(cookie_id)
...
@app.post("/cookies/validate-all")
async def validate_all_cookies_api(current_user: Dict[str, Any] = Depends(get_current_user)):
    """验证当前用户的所有Cookie（使用较短的延迟）"""
    try:
        from db_manager import db_manager
        ...
        if not user_cookies:
            return {"success": True, "message": "没有需要验证的Cookie", "results": {}}

        # 使用较短的延迟（3-8秒）进行批量验证
        validator = CookieValidator(min_delay=3, max_delay=8)
        results = {}

        for cookie_id, cookie_value in user_cookies.items():
            try:
                is_valid, message = await validator.validate_cookie(cookie_id, cookie_value)
                db_manager.update_cookie_validation_status(cookie_id, is_valid, message)
                results[cookie_id] = {"is_valid": is_valid, "message": message}
            except Exception as e:
                results[cookie_id] = {"is_valid": False, "message": f"验证出错: {str(e)}"}

        return {
            "success": True,
            "message": f"已验证 {len(results)} 个Cookie",
            "results": results
        }
    except Exception as e:
...
API_PREFIXES = ['/api/', '/static/', '/health', '/login', '/logout', '/register', '/verify', '/check-default-password', '/change-password', '/change-admin-password', '/cookies/']
