...
from utils.browser_pool import init_browser_pool, close_browser_pool
...
def _start_api_server():
    ...
    # 优先使用环境变量配置
    host = os.getenv('API_HOST', '127.0.0.1')  # 默认只绑定本地接口，避免直接暴露到公网
...
async def main():
    ...
    logger.info("浏览器实例池初始化完成")

    # 启动Cookie验证服务
    print("启动Cookie验证服务...")
    from utils.cookie_validator import start_cookie_validation
    # 使用保守的设置：每4小时检查一次，检查间隔30-120秒随机延迟
    start_cookie_validation(check_interval=14400, min_delay=30, max_delay=120)
    logger.info("Cookie验证服务已启动（每4小时检查一次，顺序检查避免并发）")
